...
class AudioEngine:
            ...
            while self.continuous_running:
                ...
                while self.continuous_running:
                    ...
                    if volume > 0.02:
                        if not is_speaking:
                            print(
                                f"AudioEngine: Début de parole détecté (vol: {volume:.3f})"
                            )
                        is_speaking = True
                        silence_chunks = 0
                    elif is_speaking:
                ...
                if is_speaking and self.continuous_running:
                    ...
                    if len(audio_concat) > self.sample_rate * 0.5:
                        print(
                            "AudioEngine: Fin de parole. Envoi au pipeline bloquant..."
                        )
    ...
    def stop_recording(self) -> str:
        ...
        if volume < 0.005:
            print(
                "AudioEngine: AVERTISSEMENT : Aucun son detecte (Volume tres faible). Verifiez que votre micro n'est pas muet."
            )
        ...
        write(wav_path, self.sample_rate, audio_int16)
        print(
            f"AudioEngine: Enregistrement sauvegardé dans {wav_path} (Volume max: {volume:.4f})."
        )
        return wav_path
        ...
        if self.whisper_model is None or self.current_whisper_model != actual_model:
            ...
            try:
                self.whisper_model = WhisperModel(
                    actual_model, device="cuda", compute_type="float16"
                )
                print("AudioEngine: Whisper charge sur GPU (CUDA) avec succes.")
            except Exception as e:
                print(
                    f"AudioEngine: CUDA indisponible ou echec ({e}). Fallback sur CPU (LENT)..."
                )
                self.whisper_model = WhisperModel(
    ...
    def preload_bark_model(self, callback_done=None):
        if self.bark_model is None:
            print(
                "AudioEngine: Téléchargement/Chargement de Bark en cours (Patientez, fichiers lourds)..."
            )
            ...
            self.bark_model = BarkModel.from_pretrained(
                "suno/bark-small", torch_dtype=dtype
            ).to(device)
    ...
    def process_tts(self, text: str, voice_pref: str = "Voice 1"):
        ...
        if "Piper" in voice_pref:
            ...
            try:
                ...
                if audio_arrays:
                    ...
                    write(
                        wav_path_piper, self.piper_voice.config.sample_rate, audio_data
                    )
                    print(
                        f"AudioEngine: Fichier Piper généré ({len(audio_data)} samples)."
                    )

                    print("AudioEngine: Lecture Piper en cours...")
                    play_audio(wav_path_piper)
                return
            except Exception as e:
                print(
                    f"AudioEngine: Erreur avec Piper TTS ({e}). Fallback sur Windows TTS."
                )
        ...
        if "Bark" in voice_pref:
            ...
            try:
                ...
                inputs = self.bark_processor(text, voice_preset="v2/fr_speaker_1").to(
                    device
                )
                ...
                print("AudioEngine: Lecture Bark en cours...")
                play_audio(wav_path_bark)
                return
            except Exception as e:
                print(
                    f"AudioEngine: Erreur avec Bark TTS ({e}). Fallback sur Windows TTS."
                )
        ...
        try:
            ...
            print(
                f"AudioEngine: Lecture TTS en cours (Utilisation de la voix : {voice_name})..."
            )
            engine.say(text)
            engine.runAndWait()
        finally:
