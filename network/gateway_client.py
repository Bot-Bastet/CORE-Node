        ...
        while True:
            ...
            try:
                ...
                async with websockets.connect(
                    url_to_use, additional_headers=headers, ssl=use_ssl
                ) as ws:
                    self.websocket = ws
                    ...
                    event_task = asyncio.create_task(self._reconnect_event.wait())
                        ...
                        [listen_task, event_task], return_when=asyncio.FIRST_COMPLETED
                    )
                    ...
                    if data.get("type") == "feature_ack":
                        ...
                        async def process_llm_request():
                            try:
                                ...
                                self.app.add_log(
                                    f"📤 Envoi de la réponse chat : '{response_text}'"
                                )
                                self.app.add_gateway_log(
                                    f'🤖 Bastet :\n"{response_text}"'
                                )
    ...
    def send_feature_request(self, feature: str, state: bool):
        ...
        if self.websocket and hasattr(self.app, "loop"):
            asyncio.run_coroutine_threadsafe(
                self.send_message(json.dumps(req)), self.app.loop
            )
